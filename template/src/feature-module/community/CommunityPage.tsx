...
const CommunityPage: React.FC = () => {
  ...
  const submitPost = async () => {
    ...
    setFormError('');
    try {
      const req: CreatePostRequest = { title: formTitle.trim(), content: formContent.trim(), type: formType };
      await communityService.createPost(req);
      setModalOpen(false);
      setPage(0);
      fetchPosts(0, filterType, search);
    } catch {
  ...
  // ── filter change ─────────────────────────────────────────────────────────
  const handleFilterType = (t: PostType | '') => {
    setFilterType(t);
    setPage(0);
  };
    ...
    <LuxuryDashboardLayout>
      <div className="container-fluid py-4">
        ...
        <div className="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
          <div>
            <h4 className="fw-bold mb-1" style={{ color: 'var(--lx-gold, #D4AF37)' }}>Community</h4>
            ...
            {posts.map((post) => (
              <div key={post.id} className="col-12">
                <div
                  ...
                  onClick={() => navigate(`${all_routes.communityPost.replace(':postId', String(post.id))}`)  }
                  onMouseEnter={(e) => (e.currentTarget.style.boxShadow = '0 4px 20px rgba(212,175,55,.18)')}
                ...
                >
                  <div className="card-body p-4">
                    <div className="d-flex align-items-start justify-content-between gap-2 mb-2">
                      ...
                      <span className={`badge bg-${TYPE_BADGE[post.type]}-subtle text-${TYPE_BADGE[post.type]} rounded-pill px-3`}>
                    ...
                    <div className="d-flex align-items-center gap-3">
                      <button
                        type="button"
                        className={`btn btn-sm d-flex align-items-center gap-1 ${post.isLiked ? 'btn-danger' : 'btn-outline-secondary'}`}
                        onClick={(e) => toggleLike(post, e)}
                      >
                        <i className={`isax ${post.isLiked ? 'isax-heart5' : 'isax-heart'} fs-14`} />
                        {post.likesCount}
                      </button>
        ...
        <div className="d-flex flex-column gap-3 pt-2">
          ...
          <div>
            ...
            <textarea
              className="form-control"
              rows={6}
              placeholder="Share your thoughts, question, or showcase…"
              value={formContent}
